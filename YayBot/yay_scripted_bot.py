...
class MyScriptedBot(ProcBot):
    ...
    def reroll(self, game):
        ...
        """
        # print("YSB {}: at top of reroll choice".format(self.id))
        reroll_proc = game.get_procedure()
        context = reroll_proc.context
        # print("YSB {}: reroll_proc {} and context {}".format(self.id, reroll_proc, context))
        # print("YSB {}: available actions are {}".format(self.id, game.get_available_actions()))
        ...
        if type(context) == botbowl.Block:
            ...
            if attackers_down == len(context.roll.dice) and context.favor != self.opp_team:
                return Action(ActionType.USE_REROLL)
            # print("YSB {}: returning DONT_USE_REROLL".format(self.id))
            # import traceback
...
# botbowl.register_bot('scripted_default', MyScriptedBot)