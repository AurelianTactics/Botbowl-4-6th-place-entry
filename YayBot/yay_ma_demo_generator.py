...
# scripted bot provided by botbowl
...
parser.add_argument("--save-directory", type=str, default='demos-ma-for-bc')
...
def main():
    # TEST need code to load model and test it
    # later: could get action probs but i don't think it really matters

    save_directory = "ray_results/{}-{}-{}-{}-{}".format(args.save_directory, args.bot_type,
                                                                   args.botbowl_size,
                                                                   args.reward_function, int(time.time()))
    ...
    if args.bot_type == "scripted" and args.botbowl_size != 11:
        print("ERROR: scripted only works with size 11")
    elif args.bot_type == "model":
        if args.load_directory is None:
            print("ERROR: no load directory set to load model ")
            quit()
        ...
        # load agent and run in evaluation mode to get examples. could do step mode but this is faster
        agent = ppo.PPOTrainer(env="my_bot_bowl_env", config=restore_config)
        agent.restore(args.load_directory)
        for i in range(args.num_demos):
            agent.evaluate() # might have to do agent.train
        print(f"Agent loaded from saved model at {args.load_directory}")

        print("Demonstrations from agent saved, quitting")
        quit()

    # build scripted bot
    env = MultiAgentBotBowlEnv(**env_config)
    HOME_PLAYER_AGENT_ID = 0
    AWAY_PLAYER_AGENT_ID = 1

    prep = get_preprocessor(env.observation_space)(env.observation_space)
    print("The preprocessor is", prep)

    # batch_builder = SampleBatchBuilder()  # or MultiAgentSampleBatchBuilder
    #policy_map: Dict[PolicyID, Policy],clip_rewards: bool,
    policy_map = {"main": PolicySpec(), "random": PolicySpec(policy_class=RandomPolicy)} # {0:"main", 1:"random"} #{"main":0, "random":1} # i'm not sure what order it should be in
    #policy_map (Dict[str,Policy]): Maps policy ids to policy instances.
    batch_builder = MultiAgentSampleBatchBuilder(policy_map=policy_map, clip_rewards=False)
    #https://github.com/ray-project/ray/blob/master/rllib/evaluation/sample_batch_builder.py#L75

    # writer = JsonWriter(
    #     os.path.join(ray._private.utils.get_user_temp_dir(), "bc_scripted_td_reward_test_{}".format(int(time.time()))))
    writer = JsonWriter(save_directory)
    test_bot = MyScriptedBot("env_bot")

    num_episodes = args.num_demos
    time_step = 0
    for episode_id in range(num_episodes):
        obs = env.reset()
        current_player = env._get_current_player()
        prior_player = current_player
        test_bot.new_game(env.env.game, env.env.env.home_team)
        mask = obs["action_mask"]

        prev_action = np.zeros((mask.shape[0],), dtype="float32")
        prev_reward = 0.
        #prev_action = {0: np.zeros((mask.shape[0],), dtype="float32"), 1: np.zeros((mask.shape[0],), dtype="float32")}
        #prev_reward = {0: 0., 1: 0.}
        done = {"__all__": False}

        while not done["__all__"]:
            aa = np.where(mask > 0.0)[0]
            if current_player == AWAY_PLAYER_AGENT_ID or (args.bot_type == "random" and current_player == HOME_PLAYER_AGENT_ID):
                ...
                try:
                    ...
                    action_bb = test_bot.act(env.env.game)
                    action = env.env.env._compute_action_idx(action_bb, flip=False)
                    action_prob = 0.995
                    ...
                    #             print("--- Episode step {} ---".format(ep_len))
                    #             print("WARNING: unable to get action index ", e)
                    #             print("attempted action (maybe_from_last turn) ", action_bb)
                    #             print("procedure before action is ", env.game.get_procedure())

                    #             for m in aa:
                    #                 print("available actions are ", m, env._compute_action(m))
                    #             print("choosing random action")
                    action = np.random.choice(aa, 1)[0]
                    action_prob = 1. / len(aa)
                    # print("testing new action prob ", action_prob, len(aa), np.log(action_prob))
                        # is_exception = False  # turn this on for debugging

            action_dict = {current_player: action}
            # print("action is {} action prob is {} ".format(action, action_prob))
            new_obs, reward, done, info = env.step(action_dict)
            # if reward != 0:
            #     print("Reward is not 0 {} {} -- {}".format(reward, env.env.game.state.home_team.state.score,
            #                                                env.env.game.state.away_team.state.score))
            ...
            # multi agent way
            # unclear how much to pack/unpack the dicts
            # def add_values(self, agent_id: AgentID, policy_id: AgentID, **values: Any)
            # unclear to me how to unpack from MA env
            # unclear how to do action whether it needs to be dict or not
            # unclear how to do prev_action when back and forth between agents or just whatever as done last
                # for now doing it by whatever the previous one was. not player specific

            # for now, current stuff is from the env dict's and thus needs to be accessed by current_player
            # past obs/reward/actions are waht was put in last time
            values_dict = {
                "t": time_step,
                "eps_id": episode_id,
                "agent_index": current_player,
                ...
                #"rewards": reward,
                "rewards": reward[current_player],
                "prev_actions": prev_action,
                #prev_actions": prev_action[prior_player],
                "prev_rewards": prev_reward,
                #prev_rewards": prev_reward[prior_player],
                #"dones": done,
                "dones": done["__all__"],
                "infos": info,
                #"new_obs": prep.transform(new_obs)
                "new_obs": prep.transform(new_obs[current_player])
            }
            policy_id_dict = {0: "main", 1: "random"}  # maybe used in batch builder? idk how to do it
            batch_builder.add_values(agent_id=current_player, policy_id=policy_id_dict[current_player], **values_dict)

            # single agent env way, unclear for multi agent
            ...
            #     infos=info,
            #     new_obs=prep.transform(new_obs))
            obs = new_obs[current_player]
            prev_action[current_player] = action
            prev_reward[current_player]  = reward[current_player]
            prior_player = current_player
            time_step += 1

        writer.write(batch_builder.build_and_reset())
        print("final score is {} {} ".format(env.env.game.state.home_team.state.score,
                                             env.env.game.state.away_team.state.score))
